...
class TtyrecEnvPool:
    def __init__(self, flags, **dataset_kwargs):
        ...
        self.dataset = dataset.TtyrecDataset("autoascend", **dataset_kwargs)
        ...
        self.screen_shape = (flags.ttyrec_batch_size, flags.ttyrec_unroll_length) + obs[
            "screen_image"
        ].shape

        self._iterators = [
            self.make_single_iter(self.dataset)
            for _ in range(flags.ttyrec_envpool_size)
        ]
        self._results = []
        for i in self._iterators:
            self._results.append(self.threadpool.submit(next, i))

    def result(self):
        return self._results[self.idx].result()

    def step(self):
        self._results[self.idx] = self.threadpool.submit(
            next, self._iterators[self.idx]
        )
...
def make_ttyrec_envpool(threadpool, flags):
    dbfilename = "/private/home/raileanu/fair/workspaces/clean_rl/ttyrecs.db"
...
def compute_value_loss(diff, expectile=0.8):
...
def compute_inverse_loss(predicted_action_logits, actions):
    ...
    return torch.nn.functional.cross_entropy(
        predicted_action_logits.view(T * B, -1), actions.view(T * B)
    )
...
def compute_gradients(data, learner_state, stats):
    ...
    if FLAGS.supervised_loss or FLAGS.behavioural_clone:
        ...
        TTYREC_HIDDEN_STATE[idx] = (t.detach() for t in TTYREC_HIDDEN_STATE[idx])

        true_a = torch.flatten(ttyrec_data["actions_converted"], 0, 1)
        logits = torch.flatten(ttyrec_predictions["policy_logits"], 0, 1)

        supervised_loss = FLAGS.supervised_loss * F.cross_entropy(logits, true_a).mean()
        ...
        if FLAGS.use_inverse_model:
            inverse_loss = FLAGS.inverse_loss * compute_inverse_loss(
                ttyrec_predictions["inverse_action_logits"],
                ttyrec_data["actions_converted"].long(),
            )
...
def compute_gradients_dqn(data, learner_state, stats):
    global TTYREC_ENVPOOL, TTYREC_HIDDEN_STATE, TTYREC_RUNNING_REWARD
    ...
    if FLAGS.use_online_data:
        ...
        dones = env_outputs["done"][:-1]
        actions = actor_outputs["action"][:-1]
        ...
        qvalues = learner_outputs["qvalue"][:-1]  # current state
        target_qvalues = learner_outputs["target_qvalue"][1:]  # next state

        # Reshaping
        T, B, A = qvalues.shape
        dones = dones.reshape(T * B)
        rewards = rewards.reshape(T * B)
        actions = actions.reshape(T * B, 1)
        qvalues = qvalues.reshape(T * B, A)
        target_qvalues = target_qvalues.reshape(T * B, A)

        # Compute Q-values
        actions_onehot = torch.zeros(T * B, A).to(FLAGS.device)
        actions_onehot.scatter_(1, actions, 1)
        state_action_values = qvalues.gather(1, actions)
    ...
    if FLAGS.use_offline_data:
        data = TTYREC_ENVPOOL.result()
        idx = TTYREC_ENVPOOL.idx
        ...
        TTYREC_RUNNING_REWARD *= (~dones).float()
...
def compute_gradients_cql(data, learner_state, stats):
    global TTYREC_ENVPOOL, TTYREC_HIDDEN_STATE, TTYREC_RUNNING_REWARD
    ...
    if FLAGS.use_online_data:
        ...
        total_loss += FLAGS.cql_loss_coef * cql_loss
        stats["cql_loss"] += cql_loss.item()
    ...
    if FLAGS.use_offline_data:
        ...
        ttyrec_cql_loss = (logsumexp - data_values).mean()
        total_loss += FLAGS.ttyrec_loss_coef * FLAGS.cql_loss_coef * ttyrec_cql_loss
        stats["ttyrec_cql_loss"] += ttyrec_cql_loss.item()
...
def compute_gradients_iql(data, learner_state, stats):
    global TTYREC_ENVPOOL, TTYREC_HIDDEN_STATE, TTYREC_RUNNING_REWARD
...
def main(cfg):
    ...
    stats = {
        ...
        "ttyrec_mean_square_discounted_running_reward": StatMean(cumulative=True),
        "reward_normalised": StatMean(),
        "ttyrec_reward_normalised": StatMean(),
        "running_advantages": StatMean(cumulative=True),
        "sample_advantages": StatMean(),
        "dqn_loss": StatMean(),
        "ttyrec_dqn_loss": StatMean(),
        "cql_loss": StatMean(),
        "ttyrec_cql_loss": StatMean(),
        "iql_actor_loss": StatMean(),
        "iql_value_loss": StatMean(),
        "iql_critic_loss": StatMean(),
        "ttyrec_iql_actor_loss": StatMean(),
        "ttyrec_iql_value_loss": StatMean(),
        "ttyrec_iql_critic_loss": StatMean(),
    }
